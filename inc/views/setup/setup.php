...
$config = require $publicRoot . '/inc/core/config.php';
require_once $publicRoot . '/inc/helpers/helpers.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_check()) {
        ...
        if ($action === 'save_secrets') {
            ...
            if (empty($errors)) {
                try {
                    $dbSecretsPath = secrets_root_path('db_credentials.php');
                    $emailSecretsPath = secrets_root_path('email_secret.php');
                    $appSecretsPath = secrets_root_path('app_secret.php');
                    ...
                    $config = require public_root_path('inc/core/config.php');
                    $pdo = setup_connect($config);
                    $setupRequired = app_setup_required($pdo);
                    $userCount = setup_user_count($pdo);
        ...
        if ($action === 'run_migrations') {
            if ($pdo) {
                try {
                    [$applied, $skipped] = setup_run_migrations($pdo, public_root_path('migrations'));
...
$secretsReady = is_file(secrets_root_path('db_credentials.php'))
    && is_file(secrets_root_path('email_secret.php'))
    && is_file(secrets_root_path('app_secret.php'));
...
?>
<?php render_layout_header('setup'); ?>
...
<?php render_layout_footer('setup'); ?>
