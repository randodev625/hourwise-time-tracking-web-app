...
function secrets_root_path(string $path = ''): string {
    $root = dirname(public_root_path()) . '/secrets';
    if ($path === '') {
        return $root;
    }
...
function app_routes(): array {
    static $routes = null;
    if ($routes === null) {
        $routes = require public_root_path('inc/core/routes.php');
    }
...
function app_routes_by_script_path(): array {
    ...
    $routesByScriptPath = [];
    $baseDir = str_replace('\\', '/', public_root_path());
...
function current_route_option(string $key, mixed $default = null): mixed {
    $route = current_route();
    if ($route === null) {
        return $default;
    }
    ...
    $canonicalName = trim((string)($route['canonical'] ?? ''));
    if ($canonicalName === '') {
        return $default;
    }

    $canonicalRoute = app_routes_by_name()[$canonicalName] ?? null;
    if (!is_array($canonicalRoute) || !array_key_exists($key, $canonicalRoute)) {
        return $default;
    }

    return $canonicalRoute[$key];
}

function current_route_layout(): string {
    $layout = trim((string)current_route_option('layout', 'app'));
    return $layout !== '' ? $layout : 'app';
}

function current_route_title(): string {
    return trim((string)current_route_option('title', ''));
}

function page_title(): string {
    global $page_title;

    $override = isset($page_title) ? trim((string)$page_title) : '';
    if ($override !== '') {
        return $override;
    }

    $title = current_route_title();
    return $title !== '' ? $title : 'HourWise';
}

function page_heading(): string {
    global $page_heading;

    $override = isset($page_heading) ? trim((string)$page_heading) : '';
    if ($override !== '') {
        return $override;
    }

    $heading = trim((string)current_route_option('heading', ''));
    if ($heading !== '') {
        return $heading;
    }
...
function page_subtitle(): string {
    global $page_subtitle;

    $override = isset($page_subtitle) ? trim((string)$page_subtitle) : '';
    if ($override !== '') {
        return $override;
    }

    return trim((string)current_route_option('subtitle', ''));
}

function page_message(): string {
    global $page_message;

    $override = isset($page_message) ? trim((string)$page_message) : '';
    if ($override !== '') {
        return $override;
    }

    $message = trim((string)current_route_option('message', ''));
    if ($message !== '') {
        return $message;
    }
...
function page_body_class(): string {
    global $page_body_class;

    return trim((string)($page_body_class ?? ''));
}

function document_title(): string {
    $title = page_title();
    return $title === 'HourWise' ? 'HourWise' : $title . ' - HourWise';
}

function layout_header_path(?string $layout = null): string {
    $layout = trim((string)($layout ?? current_route_layout()));

    return match ($layout) {
        'auth' => public_root_path('inc/layout/auth_header.php'),
        'error' => public_root_path('inc/layout/error_header.php'),
        'setup' => public_root_path('inc/layout/setup_header.php'),
        default => public_root_path('inc/layout/header.php'),
    };
}

function layout_footer_path(?string $layout = null): string {
    $layout = trim((string)($layout ?? current_route_layout()));

    return match ($layout) {
        'auth' => public_root_path('inc/layout/auth_footer.php'),
        'error' => public_root_path('inc/layout/error_footer.php'),
        'setup' => public_root_path('inc/layout/setup_footer.php'),
        default => public_root_path('inc/layout/footer.php'),
    };
}

function render_layout_header(?string $layout = null): void {
    require layout_header_path($layout);
}

function render_layout_footer(?string $layout = null): void {
    require layout_footer_path($layout);
}
...
function current_running_timer(): ?array {
    static $runningTimer = false;
    ...
    ');
    $stmt->execute([$userId]);
    $runningTimer = $stmt->fetch(PDO::FETCH_ASSOC) ?: null;

    return $runningTimer;
}

function app_log_path(string $filename): string {
    return secrets_root_path('logs/' . ltrim($filename, '/'));
}
...
function avatar_file_path(?string $path): ?string {
    ...
    $dir = realpath(public_root_path('uploads/avatars'));
    if ($dir === false) {
        return null;
    }
...
function base_url(string $path = ''): string {
    $cfg = require public_root_path('inc/core/config.php');
...
function require_admin(): void {
    require_login();
    if (!is_admin_user()) {
        $_SERVER['HOURWISE_ROUTE_NAME'] = 'error_403';
        $_SERVER['HOURWISE_ROUTE_PATH'] = '/403';
        $_SERVER['HOURWISE_ROUTE_PARAMS'] = [];
        require public_root_path('inc/views/errors/status.php');
        exit;
    }
}
